                    ...
                    if(selected_charges_name != "" && selected_action != "") {
                        if(jQuery('input[name="charges_names[]"]').length > 0) {
							jQuery('.charges_table tbody').find('tr').each(function () {
								...
								prev_charges_name_lower = prev_charges_name.trim();
		...
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
                ...
                if(jQuery('select[name="group"]').is(":visible")) {
                    if(jQuery('select[name="group"]').length > 0) {
                        ...
                        if(typeof group == "undefined" || group == "" || group == 0) {
							jQuery('select[name="group"]').parent().after('<span class="infos">Select the Unit</span>');
                            unit_sub_error = 0;
                        }
                    }
                }
                ...
                if(jQuery('input[name="selected_content"]').length > 0) {
					...
					if(selected_unit_type == '2' && (typeof selected_content == "undefined" || selected_content == "" || selected_content == 0)) {
                        all_errors_check = 0;
                    }
                    else if(selected_unit_type == '2' && numbers_regex.test(selected_content) == false) {
                        all_errors_check = 0;
                    }
                ...
                if(parseFloat(all_errors_check) == 1 && parseFloat(unit_sub_error) == 1) {
                    ...
                    if(location != "" && selected_stock_date != "") {
                        if(jQuery('input[name="location_id[]"]').length > 0) {
								...
								jQuery('.product_stock_table tbody').find('tr').each(function () {
									...
									prev_stock_date = jQuery(this).find('input[name="stock_date[]"]').val();
									if (prev_location_id == location && prev_stock_date == selected_stock_date) {
										add = 0;
									}
								});
							}
                    ...
                    if(parseFloat(add) == 1) {
                        ...
                        jQuery('.product_stock_table').before('<span class="infos w-50 text-center mb-3" style="font-size: 15px;">This Location & Type Already Exists</span>');
                    }
                }
                else {
	...
	jQuery.ajax({
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
				...
				var godown = "";
                if(jQuery('select[name="godown"]').is(":visible")) {
                    if(jQuery('select[name="godown"]').length > 0) {
                        godown = jQuery('select[name="godown"]').val();
								...
								if(jQuery('input[name="unit_type[]"]').length > 0) {
									jQuery('.product_consumption_table tbody').find('tr').each(function () {
										...
										if (prev_godown_id == godown && prev_product_id == product) {
											console.log("prev_unit_type : "+prev_unit_type);
											console.log('unit_type : '+selected_unit_type);
											console.log("perv_quantity : "+perv_quantity);
											console.log('selected_quantity : '+selected_quantity);
											console.log("perv_subunit_contains : "+perv_subunit_contains);
											console.log("selected_consumption_content : "+selected_consumption_content);
											console.log('unit_type : '+selected_unit_type);
											console.log('limit : '+limit);
											if(prev_unit_type == 1) {
				...
				if(parseFloat(limit_errors_check) == 1 && parseFloat(all_errors_check) == 1 && parseFloat(unit_sub_error) == 1) {
                    var add = 1;
                    if(product != "" && selected_unit_type != "" && godown != "") {
                        if(jQuery('input[name="godown_id[]"]').length > 0) {
							if(jQuery('input[name="product_id[]"]').length > 0) {
					...
					if(parseFloat(add) == 1) {
                        ...
                        product_count = parseInt(product_count) + 1;
                        jQuery('input[name="product_count"]').val(product_count);
                        var post_url = "action_changes.php?product_consumption_row_index="+product_count+"&godown="+godown+"&product="+product+"&selected_unit_type="+selected_unit_type+"&selected_quantity="+selected_quantity+"&unit_subunit="+globalVar+"&selected_consumption_content="+selected_consumption_content;
                        jQuery.ajax({
                            url: post_url, success: function (result) {
								console.log(result);
			...
			if (check_login_session == 1) {
				...
				if(parseFloat(all_errors_check) == 1 && parseFloat(unit_sub_error) == 1) {
					...
					if(parseFloat(add) == 1) {
						...
						var post_url = "action_changes.php?purchase_entry_row_index="+product_count+"&product="+product+"&unit_type="+selected_unit_type+"&quantity="+quantity+"&content="+content+"&rate="+rate+"&per="+per+"&per_type="+purchase_per_type+"&amount="+amount+"&unit_subunit="+globalVar;
