		...
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
				if (jQuery('#excel_upload_details_table').find('tbody').find('tr:nth-child(' + row_index + ')').length > 0) {
					...
					var case_rate = "";
					if (jQuery(excel_row).find('input[name="case_rate"]').length > 0) {
						case_rate = jQuery(excel_row).find('input[name="case_rate"]').val();
						case_rate = clean_value(case_rate);
						case_rate = jQuery.trim(case_rate);
					}
