<?php
	include("angalaeswari_fireworks_16042025.php");
	class Basic_Functions extends db {
		...
		public function getProjectTitle() {
			...
			if(!empty($company_name) && $company_name != $GLOBALS['null_value']) {
				$project_title = html_entity_decode($this->encode_decode('decrypt', $company_name));
			}
			if(empty($project_title)) { $project_title = "Angalaeswari Fireworks"; }
		...
		public function getOtherCityList($district) {
			$company_query = ""; $agent_query = ""; $suplier_query = ""; $customer_query = "";
			$select_query = ""; $list = array();
			...
			$supplier_query = "SELECT DISTINCT(city) as others_city FROM ".$GLOBALS['supplier_table']." WHERE district = '".$district."' AND city != '".$GLOBALS['null_value']."' ORDER BY id DESC";
