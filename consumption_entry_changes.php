        ...
        if(!empty($final_array) && empty($valid_consumption))
        {
            foreach($final_array as $data)
            {
                ...
                if($current_stock_unit < 0) {
                    ...
                    if($negative_stock !='1') {
                        if($subunit_need == 1) {
                            $valid_stock = "Max stock for <b>" . $product_name . "</b> with " .  $unit_name . " & " . (!empty($data['consumption_content'] && $data['consumption_content'] != "NULL") ? ($data['consumption_content'] . " " . $sub_unit_name ) : "") . "<br>Current Stock : " . $available_stock_unit . " " . $unit_name . " & " . $available_stock_subunit . " " . $sub_unit_name;
                            $stock_error = 1;
                        } else {
