			...
			if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                for($i = 0; $i< count($lower_case_name); $i++) {	
                    if(!empty($lower_case_name[$i])) {
                        $prev_payment_mode_id = $obj->CheckPaymentModeAlreadyExists('', $lower_case_name[$i]);
                ...
                if(!empty($edit_id))
                {
                    $prev_payment_mode_id = $obj->CheckPaymentModeAlreadyExists('', $single_lower_case_name);
                    if(!empty($prev_payment_mode_id)) {
				...
				if(empty($edit_id)) {
					for($p= 0; $p < count($payment_mode_name); $p++) {
						if(empty($payment_mode_error)) {						
							...
							$columns = array('created_date_time', 'creator', 'creator_name', 'payment_mode_id', 'payment_mode_name', 'lower_case_name', 'deleted');
							$values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$null_value."'", "'".$payment_mode_name[$p]."'", "'".$lower_case_name[$p]."'", "'0'");
