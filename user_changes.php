...
?>
        <form class="poppins pd-20 redirection_form" name="user_form" method="POST">
            ...
            <div class="row p-3">
                ...
                <div class="col-lg-3 col-md-4 col-12">
                    <div  id="password_cover" class="form-group mb-2">
                        <div class="form-label-group in-border">
    ...
    if(isset($_POST['name'])) {	
        ...
        if(empty($valid_user) && empty($access_permission_error)) {
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                if(!empty($edit_id)) {
                    $factory_id = $obj->getTableColumnValue($GLOBALS['user_table'], 'user_id', $edit_id, 'factory_id');
                    $godown_id = $obj->getTableColumnValue($GLOBALS['user_table'], 'user_id', $edit_id, 'godown_id');
                    $magazine_id = $obj->getTableColumnValue($GLOBALS['user_table'], 'user_id', $edit_id, 'magazine_id');
                    ...
                    $factory_details = $obj->getTableColumnValue($GLOBALS['factory_table'], 'id', $factory_unique_id, 'factory_details');
                    $godown_details = $obj->getTableColumnValue($GLOBALS['godown_table'], 'id', $godown_unique_id, 'godown_details');
                    $magazine_details = $obj->getTableColumnValue($GLOBALS['magazine_table'], 'id', $magazine_unique_id, 'magazine_details');
                }
                if(!empty($factory_details) && $factory_details != $GLOBALS['null_value']) {
                    $factory_details = $obj->encode_decode('decrypt', $factory_details);
                    $factory_details = explode("<br>", $factory_details);
                    if(strpos($factory_details[count($factory_details) - 1], "Incharge") !== false) {
                        $deleted_value = "";
                        $deleted_value = array_pop($factory_details);
                    }
                    $factory_details = implode("<br>",$factory_details);
                    $factory_details = $factory_details."<br>Incharge - ".$name_mobile;
                ...
                if(!empty($magazine_details) && $magazine_details != $GLOBALS['null_value']) {
                    $magazine_details = $obj->encode_decode('decrypt', $magazine_details);
                    $magazine_details = explode("<br>", $magazine_details);
                    if(strpos($magazine_details[count($magazine_details) - 1], "Incharge") !== false) {
                        $deleted_value = "";
                        $deleted_value = array_pop($magazine_details);
                    }
                    $magazine_details = implode("<br>",$magazine_details);
                    $magazine_details = $magazine_details."<br>Incharge - ".$name_mobile;
                ...
                if(empty($edit_id)) {
                    if(empty($prev_user_id)) {
                        ...
                        $columns = array('created_date_time', 'creator', 'creator_name', 'user_id', 'name', 'mobile_number', 'name_mobile', 'password', 'admin', 'type', 'factory_id', 'godown_id', 'magazine_id', 'role_id', 'login_id', 'lower_case_login_id','deleted');
                        $values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$null_value."'", "'".$name."'", "'".$mobile_number."'", "'".$name_mobile."'",  "'".$password."'", "'".$admin."'", "'".$type."'", "'".$null_value."'", "'".$null_value."'", "'".$null_value."'", "'".$role_id."'","'".$login_id."'", "'".$lower_case_login_id."'", "'0'");
                    ...
                    if(empty($prev_user_id) || $prev_user_id == $edit_id) {
                        ...
                        if(preg_match("/^\d+$/", $getUniqueID)) {
                            ...
                            if(preg_match("/^\d+$/", $user_update_id)) {	
                                // if(!empty($factory_unique_id) && preg_match("/^\d+$/", $factory_unique_id)) {
                                //     $columns = array(); $values = array();						
                                //     $columns = array('incharge_name', 'mobile_number', 'user_id', 'password', 'factory_details');
                                //     $values = array("'".$name."'", "'".$mobile_number."'", "'".$login_id."'", "'".$password."'", "'".$factory_details."'");
    
                                //     $factory_update_id = $obj->UpdateSQL($GLOBALS['factory_table'], $factory_unique_id, $columns, $values, '');
                                // }
                                // if(!empty($godown_unique_id) && preg_match("/^\d+$/", $godown_unique_id)) {
                                //     $columns = array(); $values = array();		
                                //     $columns = array('contact_person', 'mobile_number', 'user_id', 'password', 'godown_details');
                                //     $values = array("'".$name."'", "'".$mobile_number."'", "'".$login_id."'", "'".$password."'", "'".$godown_details."'");

                                //     $godown_update_id = $obj->UpdateSQL($GLOBALS['godown_table'], $godown_unique_id, $columns, $values, '');
                                // }
                                // if(!empty($magazine_unique_id) && preg_match("/^\d+$/", $magazine_unique_id)) {
                                //     $columns = array(); $values = array();		
                                //     $columns = array('contact_person', 'mobile_number', 'user_id', 'password', 'magazine_details');
                                //     $values = array("'".$name."'", "'".$mobile_number."'", "'".$login_id."'", "'".$password."'", "'".$magazine_details."'");

                                //     $magazine_update_id = $obj->UpdateSQL($GLOBALS['magazine_table'], $magazine_unique_id, $columns, $values, '');
                                // }
