    ...
    class Creation_functions extends Basic_Functions {
		...
		public function getDeliverySlipIndex($delivery_slip_id, $conversion_update) {
			...
			if(!empty($conversion_update) && $conversion_update == 1) {
				...
				$old_delivery_slip_query = "SELECT * FROM " . $GLOBALS['delivery_slip_table'] . " WHERE proforma_invoice_id = '" . $delivery_slip_id . "' AND cancelled = '0'";
				...
				$proforma_invoice_list = $this->getAllRecords($GLOBALS['proforma_invoice_table'], 'proforma_invoice_id', $delivery_slip_id);
				...
				$list = $this->getAllRecords($GLOBALS['delivery_slip_table'], 'delivery_slip_id', $delivery_slip_id);
		...
		public function getEstimateIndex($estimate_id, $conversion_update) {
			$list = array();
			if(!empty($conversion_update) && $conversion_update == 1) {
				$delivery_slip_list = $this->getAllRecords($GLOBALS['delivery_slip_table'], 'delivery_slip_id', $estimate_id);
