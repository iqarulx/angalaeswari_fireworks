<?php
    ...
    $pdf->SetX(115);
    $pdf->Cell(40,10,'GST Number',1,0,'C');
    $pdf->SetX(155);
    ...
    if(!empty($total_records_list)) {
        foreach($total_records_list as $key => $list) {
            ...
            if(!empty($list['supplier_name'])) {
                ...
                $pdf->SetY($start_y);
                $pdf->SetX(20);
                $pdf->MultiCell(55,6,html_entity_decode($list['supplier_name']),0,'C');
            } else {
                $pdf->SetY($start_y);
                $pdf->SetX(20);
            ...
            if(!empty($list['gst_number']) && $list['gst_number'] != $GLOBALS['null_value']) {
                $list['gst_number'] = html_entity_decode($list['gst_number']);
                $pdf->SetY($start_y);
                $pdf->SetX(115);
                $pdf->MultiCell(40,6,$list['gst_number'],0,'C');
            } else {
                $pdf->SetY($start_y);
                $pdf->SetX(115);
    ...
    $pdf->Output($from,'supplier List.pdf');
?>